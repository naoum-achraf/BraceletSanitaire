...
public class TestController {
	...
	public Optional<User>
	  getPatient(@PathVariable String name){ return user.findByUsername(name); }
	...
	public ResponseEntity<?> updateUser(@PathVariable Long id,@RequestBody User u) {
		...
		user1.setHopitale(u.getHopitale());
	...
	public List<User> nbrPatient(@PathVariable Long id) {
		return user.getNbr(id);
	}
	
	@GetMapping("/notDanger/{id}")
	public List<User> nbrAttent(@PathVariable Long id) {
		return user.getNotDanger(id);
	}
	
	@GetMapping("/danger/{id}")
	public List<User> négative(@PathVariable Long id) {
		return user.getDanger(id);
	}
