...
public class MessagerieController {
	@Autowired
	private MessagerieRepo Repo;
	
	
	@GetMapping("/messagerie/{id}")
	public List<Messagerie> négatives(@PathVariable Long id) {
		return Repo.getInfosPateint(id);
	}
	
	@GetMapping("/AllmessagesM/{id}")
	public List<Messagerie> AllMessageM(@PathVariable Long id) {
		return Repo.getMessageM(id);
	}
