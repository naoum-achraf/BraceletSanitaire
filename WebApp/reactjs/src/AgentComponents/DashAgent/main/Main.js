...
import hello from "../../assets/medical-mask.png";
...
componentDidMount(){
  ...
  axios.get("http://localhost:8080/api/test/notDanger/"+AuthService.getCurrentUser().id)
  .then(Response => Response.data)
  .then((data) => {
      this.setState({Négative: data});
  })

  axios.get("http://localhost:8080/api/test/danger/"+AuthService.getCurrentUser().id)
  .then(Response => Response.data)
  .then((data) => {
